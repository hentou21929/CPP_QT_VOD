...
void MainWid::setUrl(const QString &url)
{
     this->url=url;
     qDebug() <<"url为 -----------"<<this->url;
}
QString MainWid::getUrl()
{
     return this->url;
}
void MainWid::Setvideototaltime(int64_t totaltime)
{
     int totalTimeInt = static_cast<int>(totaltime);
     qDebug()<<"totalTimeInt==="<<totalTimeInt;
     ui->CtrlBarWid->OnVideoTotalSeconds(totalTimeInt);
     //ui->CtrlBarWid->OnVideoTotalSeconds(totaltime);
}
void MainWid::handlePlayRequest()
{
    // 获取 VideoCtl 实例
     VideoCtl *videoCtl = VideoCtl::GetInstance();

     // 获取窗口句柄
     WId windowId = ui->ShowWid->getLabelWinId();

     // 通过实例调用非静态方法
     if (videoCtl->StartPlay(this->getUrl(), windowId)) {
         qDebug() << "播放成功"<<this->getUrl();
     } else {
         QMessageBox::critical(this, "错误", "无法播放");
     }
}
// 2. 重写closeEvent
void MainWid::closeEvent(QCloseEvent *event) {
     if (VideoCtl::GetInstance()) {
         VideoCtl::GetInstance()->OnStop(); // 停止播放线程

         // 同步释放，避免异步问题
         VideoCtl::ReleaseInstance();
     }

     // 断开所有与 VideoCtl 相关的信号槽
     disconnect(VideoCtl::GetInstance(), nullptr, this, nullptr);
