...
void VideoCtl::video_refresh(void *opaque, double *remaining_time)
{
    ...
    is->force_refresh = 0;
    //qDebug()<<"onplaytime=-------"<<get_master_clock(is);
...
//        loop.exec();
    ...
    // 设置HLS特有参数
    { // 新增作用域开始
        const AVInputFormat* fmt = av_find_input_format("hls");
        if (!fmt) {
            av_log(NULL, AV_LOG_ERROR, "HLS demuxer not available\n");
            ret = -1;
            goto fail; // 现在安全跳转
        }
       ...
       std::string output_url ="http://119.91.46.232:8080/live/20250326162409531.m3u8";
        // 调试输出字符串长度和内容
        qDebug() << "----------------------------------------";
        qDebug() << "[is->filename 内容]";
        qDebug() << "- 地址: " << static_cast<void*>(is->filename); // 打印内存地址
        qDebug() << "- 字符串长度: " << strlen(is->filename);
        qDebug() << "- 内容: " << is->filename;

        qDebug() << "\n[output_url.c_str() 内容]";
        qDebug() << "- 地址: " << static_cast<const void*>(output_url.c_str());
        qDebug() << "- 字符串长度: " << output_url.size();
        qDebug() << "- 内容: " << output_url.c_str();
        qDebug() << "----------------------------------------";
        std::this_thread::sleep_for(std::chrono::seconds(10)); // 阻塞5秒

        err = avformat_open_input(&ic, is->filename, fmt, &options);
        //err = avformat_open_input(&ic, is->filename, fmt, &options);
        //err = avformat_open_input(&ic, output_url.c_str(), fmt, &options);

        av_dict_free(&options); // 关键：释放字典
        qDebug() << "is->filename:" << is->filename;
        qDebug() << "url:" << output_url.c_str();
        if (err < 0) {
...
//    m_pInstance = nullptr;
    ...
    if (m_pInstance) {
        // 确保 m_CurStream 有效且未重复释放
        m_pInstance->do_exit(m_pInstance->m_CurStream);
        delete m_pInstance;
        m_pInstance = nullptr; // 必须置空
    }
}
VideoCtl::~VideoCtl()
{
    disconnect(this, nullptr, nullptr, nullptr); // 断开所有关联信号
...
//    file_name=strFileName.tostring().c_Str();
    std::string file_name =strFileName.toStdString();
    qDebug() << "+++++++++++file_name " << QString::fromUtf8(file_name.c_str());

    //打开流
    is = stream_open(file_name.c_str());
    if (!is) {
